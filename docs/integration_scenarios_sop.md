...
| Approval status | approved (owner confirmed 2026-08-30 — end-to-end integration run authorized) |

> Pre-filled from project analysis and owner decisions (latest 2026-08-30): `docs/DEVELOPMENT_PLAN.md` §§1–9, README §"Tool Surface", `pyproject.toml`, unit suite (`test_reconciliation.py`, `test_tools.py`, `test_a2a_client.py`), `a2a_daemon_engine/models/postgresql/a2a_agent.py`. Owner-confirmed: **Hermes registered directly in `a2a_daemon_engine` and routed via its handler metadata (`module_name`/`class_name`) — NOT through `docker-a2a-hermes-agent-gateway`**; core-engine bridge as second peer; **OpenClaw removed from scope**; **PostgreSQL is the sole persistence backend under test** (registry reads verified from PostgreSQL `a2a_agents`); credentials from MCP registry settings; manual CI runs for now. **Approved 2026-08-30 for the end-to-end integration run.**
...
- **Out of scope:** daemon-side admin CRUD, operator surfaces, MCP transport/dispatch (`mcp_daemon_engine`), SSE client lifecycle, A2A protocol semantics, task persistence internals, push-notification webhooks, `resubscribe_a2a_task` (deferred to 0.2.0), remote peer card fetch (deferred, §5.1a). **This SOP does not certify `silvaengine_gateway` or `a2a_daemon_engine` as components** — they are validated only insofar as they serve this module's client paths.
- **System(s) under test:** `mcp_a2a_proxy` (target module) and its dependency platform: `silvaengine_gateway` → `a2a_daemon_engine` → peer handlers (Hermes registered in the daemon; core-engine bridge).

## 3. Environment and Access

| Item | Value / source |
|---|---|
| Environment target | local dev (`localhost:8765`, gateway confirmed listening on 2026-08-29 — probe returned 401, service up) |
...
| Credential source | per-partition MCP registry settings (confirmed by owner 2026-08-29): `token_username` / `token_password` or `x_api_key` or `gateway_token`, read at runtime; never inlined in scripts or reports |
...
| Peer: Hermes Agent (registered in `a2a_daemon_engine`, handler-routed via registry `metadata` — **not** `docker-a2a-hermes-agent-gateway`) | external | appears in `discover_a2a_agents` with status active | operational | owner (confirmed 2026-08-29) |
| Peer: `ai_agent_core_engine` bridge (in-house) | internal | reachable via daemon default routing or discovery | operational (delegation P1) | owner (confirmed 2026-08-29) |
...
|---|---|---|
| Registered A2A peer agents | 2 | Hermes (registered directly in `a2a_daemon_engine`, handler-routed via registry `metadata`) + core-engine bridge; `status=active`, in test partition; `capabilities` may be NULL — that itself is a scenario-1 assertion. Registry rows verified against PostgreSQL `a2a_agents` |
...
| **Steps** | 1. `get_a2a_agent_card()` → assert protocol version `1.0.0`, endpoint URL gateway-substituted. 2. `get_a2a_agent_card(extended=true)` → assert auth-gated acceptance. 3. Inspect client call trace: assert every call went to our own gateway base URL and nowhere else (§1 boundary). |
| **Expected behavior** | Card returned; extended card respects auth gating; no non-gateway host contacted. |
| **Validation points** | protocol_version, endpoint substitution, auth gating, boundary adherence |
| **Cross-system checks** | Card content matches daemon settings (`a2a_capabilities` etc.). |
...
| **Steps** | 1. `send_a2a_message(message=<payload>, agent_id=<peer>)` with **no prior discovery call** → `task_id`. 2. `get_a2a_task(task_id)` → terminal state with result. 3. Repeat omitting `agent_id` → daemon default agent handles it. 4. Repeat with bogus `agent_id` → `AGENT_NOT_FOUND` originates from daemon, not a proxy-side pre-flight (§5.1a). 5. Run 1–2 against the **PostgreSQL** task store (owner decision 2026-08-29: PostgreSQL is the sole persistence backend under test). |
| **Expected behavior** | Each delegation returns `task_id`; task reaches terminal state carrying result; default-agent and error paths behave per dev plan §5.1a/§7. Proxy issues exactly one `message/send` before `get_a2a_task` (excluding auth). |
| **Validation points** | `task_id` issued, terminal state reached, default-agent routing, `AGENT_NOT_FOUND` mapping, single-call discipline, PostgreSQL task store |
...
| **Steps** | 1. `send_a2a_message_stream(message=<payload>)` → collect response. 2. Assert `status: "streaming_complete"`, `events_emitted > 0`, events in order (task creation → transitions → final). |
| **Expected behavior** | Full ordered event list returned; draining drives execution to completion (dev plan §5.2). |
| **Validation points** | streaming_complete status, events_emitted, event order |
...
| **Steps** | 1. `send_a2a_message(message, agent_id=hermes)` → task reaches `input_required` (non-terminal). 2. `get_a2a_task(task_id, history_length=N)` → peer's question surfaced. 3. `send_a2a_message(message=answer, task_id=same, context_id=same)` → same task resumes. 4. `get_a2a_task(task_id)` → completed + result. 5. Run on the PostgreSQL task store (owner decision 2026-08-29). |
| **Expected behavior** | Full loop composes: pause → question surfaced → resumption on same `task_id` → completion. Decides whether `resubscribe_a2a_task` is needed (0.2.0 input). |
| **Validation points** | input_required non-terminal, question in history, same-task resumption, completion |
...
| **Steps** | 1. Simulated context loss (fresh proxy instance, same credentials). 2. `list_a2a_tasks(...)` → finds the delegation. 3. `get_a2a_task(task_id)` → state + result retrievable. |
| **Expected behavior** | Recovery works with no in-memory carryover between instances. |
| **Validation points** | task found by list, state/result via get |
| **Cross-system checks** | Task-page pagination consistent with store count (filters: `status`, `assigned_agent_id`). |
...
| **Steps** | 1. Start long delegation. 2. `cancel_a2a_task(task_id)` → accepted. 3. `get_a2a_task(task_id)` → canceled state. 4. Second `cancel_a2a_task(task_id)` → idempotent success or `TASK_ALREADY_TERMINAL`. 5. Verify no further daemon-side peer execution / no stream deliveries after cancel. |
| **Expected behavior** | Cancellation stops work in caller's partition; task row survives with canceled status; second cancel per §9 idempotency; no auto-cancel on `STREAM_TIMEOUT` (distinct path). |
| **Validation points** | cancel accepted, canceled state, idempotent second call, cascade within partition only |
...
| **Steps** | 1. daemon down → `API_CONNECTION_FAILED`. 2. daemon up, SDK uninitialized → `A2A_SDK_NOT_INITIALIZED`. 3. unknown task id → `TASK_NOT_FOUND`. 4. Assert secrets redacted from all structured error payloads (auth headers / token username/password / gateway token). |
...
|---|---|---|
| `missing_data` | unknown `task_id` / unknown `agent_id` | `TASK_NOT_FOUND` / `AGENT_NOT_FOUND` (mapped from daemon, no pre-flight) |
...
| Count consistency | delegations issued in test partition == tasks listed via `list_a2a_tasks` (page-aggregated) | 0 |
...
- Environment validated: gateway reachable and authenticated; daemon module initialized (agent card 200)
...
- All P1 scenarios (P4-1, -2, -3, -4, -7, -8) pass on tested backends; P2 scenarios (P4-6, -9) pass or are explicitly waived with justification
- §9 reconciliation checks clean
- No blocking defects; failing calls retain sanitized evidence (secrets redacted) sufficient for diagnosis
...
- Peer coverage: Hermes (daemon-registered handler) + core-engine bridge exercised; if Hermes is unreachable, certification honored accordingly — e.g. `Ready with Conditions` listing the uncovered peer as a condition, never implying untested coverage. OpenClaw is out of scope in this SOP version.
